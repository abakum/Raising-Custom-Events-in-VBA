...
#Const отладка = 1
Sub какЭтоБыло()
 Dim наган As New револьверСоднимПатроном
 Dim первый As Boolean
 Dim и As Integer
 ...
 For и = 1 To наган.сколькоЗарядный
  Вернер.ВидитЧтоРевольверВзял дуэлянты(первый + 2)
  дуэлянты(первый + 2).ПриставилДулоКвиску
  дуэлянты(первый + 2).НажалНаСпусковойКрючок
  If Not дуэлянты(первый + 2).ПередалРевольвер Then GoTo La_commedia_e_finita
  первый = Not первый
 Next и
