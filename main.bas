...
Sub какЭтоБыло()
 ...
 If 0 Then
  дуэлянты(1).ЧСС = 10 'потестим события пульс с разной частотой
  Вернер.ВидитЧтоРевольверВзял дуэлянты(1)
  Вернер.СчитаетПульс 'потестим умение доктора считать пульс на живом пациенте
 End If
...
La_commedia_e_finita:
 ...
 жди , -1 'надо запускать перед деструкцией классов из которых вызывается 'жди'
 Set Вернер = Nothing
 For i = LBound(дуэлянты) To UBound(дуэлянты)
